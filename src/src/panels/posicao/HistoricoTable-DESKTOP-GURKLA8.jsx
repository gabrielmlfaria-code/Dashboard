...
import { isArt473PreventivaEvent } from "./saude-preventiva/saudePreventivaArt473.js";
...
function computeAggregateHourMetrics(r) {
  ...
  const horasEventos = (Number(r?.horas_ausentes) || 0) + (Number(r?.horas_justificadas) || 0);
...
function computeTotals(rows, hasHours, hasExtras) {
  ...
  return {
    ...
    horas_extras: hasExtras
      ? (period?.horasExtras ?? rows.reduce((s, r) => s + (r.horas_extras || 0), 0))
      : null,
  };
}
...
export function HistoricoTable({
  ...
  const updateDateFrom = useCallback(
    (value) => {
      ...
      if (dateDraftTo && v && (new Date(dateDraftTo) - new Date(v)) / 86400000 > MAX_DAYS) {
  ...
  const updateDateTo = useCallback(
    (value) => {
      ...
      if (dateDraftFrom && v && (new Date(v) - new Date(dateDraftFrom)) / 86400000 > MAX_DAYS) {
  ...
  const minimizeFloating = useCallback(() => {
    if (onFloatMinimize) onFloatMinimize();
    else setIsFloating(false);
  }, [onFloatMinimize, setIsFloating]);

  const closeFloating = useCallback(() => {
    if (onFloatClose) onFloatClose();
    else setIsFloating(false);
  }, [onFloatClose, setIsFloating]);
  const [floatPos, setFloatPos] = useState({ x: 60, y: 30 });
  const [floatSize, setFloatSize] = useState({ w: null, h: null });
  ...
  useEffect(() => {
    if (isFloating) {
      setFloatPos({ x: 60, y: 30 });
      setFloatSize({ w: null, h: null });
    }
  }, [isFloating]);
  ...
  const filtered = useMemo(
    ...
    [enriched, effectiveDateFrom, effectiveDateTo],
  );
  ...
  const periodEvents = useMemo(
    () => sorted.flatMap((r) => (Array.isArray(r._events) ? r._events : [])),
    [sorted],
  );
  ...
  useEffect(() => {
    if (!tableViewRequest?.view && tableViewRequest?.search == null) return;
    setSelectedEmp(null);
  ...
  const modalEvents = useMemo(() => {
    ...
    return rows.flatMap((r) => (Array.isArray(r._events) ? r._events : []));
  }, [displayRows, sorted]);
  ...
  const activeDayModal = useMemo(() => {
    ...
    const events = CONFIG.ABSENTEISMO_API ? [] : hasCustomEvents ? dayModal.events : modalEvents;
  ...
  }, [deptHistRows, filtered, dateSearch, dateFrom, dateTo]);
  ...
  const openGroupModal = useCallback(
    (groupType, groupValue) => {
      ...
      const matchesGroup = (record) =>
        cleanGroupText(
          isDept ? record?.depto_desc || record?.depto : isGenero ? record?.genero : record?.cargo,
    ...
    (groupType, groupValue) => {
      ...
      const rows = deptSourceRows
        .map((row) => {
          ...
          const presentes = employees.filter(
            (employee) => (Number(employee.hrsPres) || 0) > 0,
          ).length;
          const faltas = employees.filter((employee) => (Number(employee.hrsAuse) || 0) > 0).length;
          const justificadas = employees.filter(
            (employee) => (Number(employee.hrsJust) || 0) > 0,
          ).length;
          const horasPlanejadas = employees.reduce(
            (sum, employee) => sum + (Number(employee.hrsPlan) || 0),
            0,
          );
          ...
          const horasAusentes = employees.reduce(
            (sum, employee) => sum + (Number(employee.hrsAuse) || 0),
            0,
          );
          const horasJustificadas = employees.reduce(
            (sum, employee) => sum + (Number(employee.hrsJust) || 0),
            0,
          );
          const horasExtras = employees.reduce(
            (sum, employee) => sum + (Number(employee.hrsExtr) || 0),
            0,
          );
          return {
            date: row.date,
  ...
  const deptDrillMap = useMemo(() => {
    ...
    for (const [dept, bucket] of map) {
      const criticalDays = [...bucket.days.values()]
        ...
        .slice(0, 5);
      const employees = [...bucket.employees.values()].sort(
        (a, b) =>
          b.hrsAuse + b.hrsJust - (a.hrsAuse + a.hrsJust) ||
          b.hrsExtr - a.hrsExtr ||
  ...
  const cidDrillMap = useMemo(() => {
    ...
    for (const [label, bucket] of map) {
      ...
      const topEmployees = [...bucket.employees.values()]
        .sort(
          (a, b) =>
            b.eventos - a.eventos || b.horas - a.horas || a.nome.localeCompare(b.nome, "pt-BR"),
        )
        .slice(0, 12);
  ...
  function renderDataRow(r, i) {
    ...
    const hasEmps = Array.isArray(r._employees) && r._employees.length > 0;
            ...
            if (col.id === "date")
              return (
                ...
                >
                  ...
                  {hasArt473 && (
                    <span
                      className="pb-hist-badge art473"
                      title="Ausência preventiva (art. 473, XII, CLT)"
                    >
                      473
                    </span>
                  )}
                  ...
                  >
                    ...
                    <thead>
                      <tr>
                        ...
                        {hasHours && (
                          <th className="pb-drill-th-num pb-drill-th-over">Acima plan.</th>
                        )}
                      ...
                      {r._employees.map((emp) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="pb-drill-td-text pb-drill-td-schedule" title={horario}>
                              {horario}
                            </td>
                            <td className="pb-drill-td-text pb-drill-td-punches" title={marcacoes}>
                              {marcacoes}
                            </td>
                            ...
                            {hasHours && (
                              <td
                                className={`pb-drill-td-num pb-drill-td-over${horasAcimaPlanejado > 0 ? " has-over" : ""}`}
                              >
                    ...
                    {hasHours && (
                      <tfoot>
                        <tr className="pb-drill-totals-row">
                          ...
                          <td
                            className={`pb-drill-td-num pb-drill-td-over${acimaPlanejado > 0 ? " has-over" : ""}`}
                          >
                          ...
                          {hasExtras && (
                            <td className="pb-drill-td-num">{fmtMin(drillTotals.extr)}</td>
                          )}
                        </tr>
                      </tfoot>
                    )}
      ...
      for (const row of presenceHistRows) {
        const match = (row._employees || []).find(
          (e) =>
            String(e?.nome ?? "")
              .trim()
              .toLowerCase() === nameKey && String(e?.mat ?? "").trim(),
        );
  ...
  const isPresenceProfileActive = useCallback(
    (emp) => {
      ...
      return (
        String(presenceProfileRow.nome ?? "")
          .trim()
          .toLowerCase() ===
        String(emp?.nome ?? "")
          .trim()
        ...
        <td colSpan={deptColSpan} className="pb-drill-cell">
          <div className="pb-drill-inner pb-dept-drill">
            ...
            <div className="pb-dept-drill-summary">
              <span>
                <strong>{r.colaboradoresQtd.toLocaleString("pt-BR")}</strong> colaboradores
              </span>
              <span className="is-absence">
                <strong>{r.ausentes.toLocaleString("pt-BR")}</strong> ausentes /{" "}
                {fmtMin(r.horas_ausentes)}
              </span>
              <span className="is-justified">
                <strong>{r.justificadas.toLocaleString("pt-BR")}</strong> justificadas /{" "}
                {fmtMin(r.horas_justificadas)}
              </span>
              {hasExtras && (
                <span className="is-extra">
                  <strong>{r.extras.toLocaleString("pt-BR")}</strong> extras /{" "}
                  {fmtMin(r.horas_extras)}
                </span>
              )}
              {hasHours && acimaPlanejado > 0 && (
                <span className="is-over">
                  <strong>+{fmtMin(acimaPlanejado)}</strong> acima do planejado
                </span>
              )}
            </div>
                      ...
                      {detail.criticalDays.length > 0 && (
                        <tfoot>
                          <tr className="pb-drill-totals-row">
                            ...
                            <td className="pb-drill-td-num">
                              {criticalTotals.ausentes.toLocaleString("pt-BR")}
                            </td>
                            <td className="pb-drill-td-num">{fmtMin(criticalTotals.hrsAuse)}</td>
                            <td className="pb-drill-td-num">
                              {criticalTotals.justificadas.toLocaleString("pt-BR")}
                            </td>
                            <td className="pb-drill-td-num">{fmtMin(criticalTotals.hrsJust)}</td>
                            {hasExtras && (
                              <td className="pb-drill-td-num">
                                {criticalTotals.extras.toLocaleString("pt-BR")}
                              </td>
                            )}
                            {hasExtras && (
                              <td className="pb-drill-td-num">{fmtMin(criticalTotals.hrsExtr)}</td>
                            )}
                          </tr>
                        </tfoot>
                      )}
                          ...
                          detail.employees.map((emp) => (
                            ...
                            >
                              ...
                              <td className="pb-drill-status-cell">
                                {(emp.hrsAuse || 0) > 0 && (
                                  <span className="pb-drill-status is-absence">Ausência</span>
                                )}
                                {(emp.hrsJust || 0) > 0 && (
                                  <span className="pb-drill-status is-justified">Justificada</span>
                                )}
                                {(emp.hrsExtr || 0) > 0 && (
                                  <span className="pb-drill-status is-extra">Extra</span>
                                )}
                              </td>
                              {hasHours && (
                                <td className="pb-drill-td-num">{fmtMin(emp.hrsPlan)}</td>
                              )}
                              {hasHours && (
                                <td className="pb-drill-td-num">{fmtMin(emp.hrsPres)}</td>
                              )}
                      ...
                      {detail.employees.length > 0 && (
                        <tfoot>
                          <tr className="pb-drill-totals-row">
                            ...
                            <td className="pb-drill-status-cell" />
                            {hasHours && (
                              <td className="pb-drill-td-num">{fmtMin(employeeTotals.hrsPlan)}</td>
                            )}
                            {hasHours && (
                              <td className="pb-drill-td-num">{fmtMin(employeeTotals.hrsPres)}</td>
                            )}
                            ...
                            {hasExtras && (
                              <td className="pb-drill-td-num">{fmtMin(employeeTotals.hrsExtr)}</td>
                            )}
                          </tr>
                        </tfoot>
                      )}
    ...
    return (
      <th className={className}>
        <button
          type="button"
          className={`pb-th-sort${active ? " active" : ""}`}
          onClick={() => toggleCargoSort(key)}
        >
          <span className="pb-th-label notranslate" translate="no">
            {label}
          </span>
          <span className="pb-th-arrow" aria-hidden="true">
            {active ? (cargoSortDir === "asc" ? "▲" : "▼") : "⇅"}
          </span>
        </button>
      </th>
    );
    ...
    return (
      <th className={className}>
        <button
          type="button"
          className={`pb-th-sort${active ? " active" : ""}`}
          onClick={() => toggleGeneroSort(key)}
        >
          <span className="pb-th-label notranslate" translate="no">
            {label}
          </span>
          <span className="pb-th-arrow" aria-hidden="true">
            {active ? (generoSortDir === "asc" ? "▲" : "▼") : "⇅"}
          </span>
        </button>
      </th>
    );
    ...
    return (
      <th className={className}>
        <button
          type="button"
          className={`pb-th-sort${active ? " active" : ""}`}
          onClick={() => toggleCidSort(key)}
        >
          <span className="pb-th-label">{label}</span>
          <span className="pb-th-arrow" aria-hidden="true">
            {active ? (cidSortDir === "asc" ? "▲" : "▼") : "⇅"}
          </span>
        </button>
      </th>
    );
    ...
    return (
      <th className={className}>
        <button
          type="button"
          className={`pb-th-sort${active ? " active" : ""}`}
          onClick={() => toggleRiskSort(key)}
        >
          <span className="pb-th-label">{label}</span>
          <span className="pb-th-arrow" aria-hidden="true">
            {active ? (riskSortDir === "asc" ? "▲" : "▼") : "⇅"}
          </span>
        </button>
      </th>
    );
                          ...
                          <tr key={d.dept}>
                            <td className="pb-drill-td-name">
                              <button
                                ...
                                className="pb-date-link"
                                onClick={() =>
                                  openRiscoModal({ eventLabel: r.evento, dept: d.dept })
                                }
                                title={d.dept}
                              >
                                {d.dept}
                    ...
                    {detail.topDepartments.length > 0 && (
                      <tfoot>
                        <tr className="pb-drill-totals-row">
                          <td className="pb-drill-td-name">Totais</td>
                          <td className="pb-drill-td-num">
                            {totalDepartments.toLocaleString("pt-BR")}
                          </td>
                        </tr>
                      </tfoot>
                    )}
                          ...
                          <tr key={emp.key}>
                            <td className="pb-drill-td-name">
                              <button
                                ...
                                className="pb-date-link"
                                onClick={() =>
                                  openRiscoModal({ eventLabel: r.evento, emp: emp.mat || emp.nome })
                                }
                                title={emp.nome}
                              >
                                {emp.nome}
                            ...
                            <td className="pb-drill-td-text" title={emp.dept}>
                              {emp.dept}
                            </td>
                            <td className="pb-drill-td-num">
                              {emp.eventos.toLocaleString("pt-BR")}
                            </td>
                            <td className="pb-drill-td-num">{fmtMin(emp.horas)}</td>
                          </tr>
                        ))
                    ...
                    {detail.topEmployees.length > 0 && (
                      <tfoot>
                        <tr className="pb-drill-totals-row">
                          ...
                          <td className="pb-drill-td-num">
                            {fmtMin(detail.topEmployees.reduce((s, emp) => s + emp.horas, 0))}
                          </td>
                        </tr>
                      </tfoot>
                    )}
  ...
  const colsAndFloat = (
    <>
      ...
      <button
        ...
        title={isFloating ? "Fixar no painel" : "Abrir em modal"}
    ...
    <>
      ...
      >
        ...
        <div className="pb-hist-float-actions">
          <button
            type="button"
            className="pb-hist-float-minimize"
            title="Minimizar"
    ...
    <>
      ...
      {isFloating ? (
        createPortal(
          ...
          >
            <div
              className="pos-bento pb-hist-float-panel"
              data-theme={theme}
              ref={floatPanelRef}
              style={{
                left: floatPos.x,
