...
import {
  ...
  atrasoEventsFromRows as atrasoEventsFromRowsShared,
...
import { fmtMinutes as fmtHmShared } from "./nlFormatters.js";
...
} from "../saude-preventiva/saudePreventivaArt473.js";
...
export function atrasoEventsFromRows(rows) {
  return atrasoEventsFromRowsShared(rows);
}

function fmtHm(min) {
  return fmtHmShared(min);
}
...
export function buildDashboardNlContext({
  ...
  const saudeRegistros =
    typeof window !== "undefined" ? loadSaudeRegistrosSync().map(normalizeSaudeRegistro) : [];
  ...
  return {
    ...
    periodLabel:
      periodLabel ||
      (rows.length ? `${rows[0]?.date} — ${rows[rows.length - 1]?.date}` : "Sem período"),
    dias: rows.length,
    totals: {
    ...
    entidades: {
      colaboradores: [...colaboradoresMap.values()].sort((a, b) =>
        a.nome.localeCompare(b.nome, "pt-BR"),
      ),
      departamentos: [
        ...new Set([...deptRanking.map((d) => d.dept).filter(Boolean), ...departamentosSet]),
