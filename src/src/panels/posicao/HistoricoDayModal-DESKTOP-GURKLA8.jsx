...
import { computeEventModalTotalsFromEvents, computeModalPeriodTotals } from "./radarHoursUtils.js";
import {
  buildHdmColFilter,
  HDM_FILTER_OPS,
...
export function HistoricoDayModal({
  ...
  const [stackHrsMrc, setStackHrsMrc] = useState(() => {
    ...
    if (isPosEmbedded) return posEmbeddedSaved.stackHrsMrc ?? posEmbeddedPreset.defaultStackHrsMrc;
  ...
  const [visibleCols, setVisibleCols] = useState(() => {
    ...
    const saved = isEventsMode
      ? (disk.evtVisibleCols ?? _hdmSaved.evtVisibleCols)
      : (disk.empVisibleCols ?? _hdmSaved.empVisibleCols);
    const catalog = isEventsMode
      ? (disk.evtColCatalog ?? _hdmSaved.evtColCatalog)
      : (disk.empColCatalog ?? _hdmSaved.empColCatalog);
    return resolveVisibleColSet(
      saved,
      catalog,
      COLS.map((c) => c.id),
    );
  ...
  const [colOrder, setColOrder] = useState(() => {
    if (lockColumnLayout) {
      return RISCO_EVT_COL_ORDER.filter((id) => id === "nome" || COLS.some((c) => c.id === id));
    }
    if (isPosEmbedded) return resolvePosEmbeddedColOrder(posEmbeddedSaved, posListKey);
    const disk = readStoredJson(HDM_STATE_KEY, {});
    const saved = isEventsMode
      ? (disk.evtColOrder ?? _hdmSaved.evtColOrder)
      : (disk.empColOrder ?? _hdmSaved.empColOrder);
  ...
  const [colWidths, setColWidths] = useState(() => {
    ...
    return (
      (isEventsMode
        ? (disk.evtColWidths ?? _hdmSaved.evtColWidths)
        : (disk.empColWidths ?? _hdmSaved.empColWidths)) || {}
    );
  });
  const thRefs = useRef({});
  ...
  const openFilter = (col, btnEl) => {
    ...
    setFilterCondDraft(normalized.cond ? { ...normalized.cond } : { op: "contains", value: "" });
  ...
  const filteredEventIndices = useMemo(() => {
    ...
    const useDateIndex =
      eventsDateIndex && !isSheetImportMode && !pillFilter && !q && !hasColFilters;
  ...
  const empSortKeys = useMemo(() => filteredEmps.map((e) => buildEmpSortKeys(e)), [filteredEmps]);
  ...
  const groupingTooLarge =
    ...
    !hasAnyFilter;
  const tableBusy = dateFilterBusy || groupBusy || sortBusy || (isApiMode && apiData.isFetching);
  ...
  const catCounts = useMemo(() => {
    ...
    if (indices) {
      ...
      for (let idx = 0; idx < events0.length; idx++) {
        const ev = events0[idx];
        if (!ev._cat) continue;
        const evDate = normDateKey(ev.data);
        if (fromKey && evDate && evDate < fromKey) continue;
        if (toKey && evDate && evDate > toKey) continue;
        loop(idx);
      }
      ...
      return (
        <>
          {nodes
            .slice(0, limit)
            .map((rowIdx, i) =>
              renderLeafRow(tableDataRows[rowIdx], rowIdx, `${pathPrefix}${rowIdx}-`),
            )}
          {limited && (
            <tr>
    ...
    return nodes.map((node) => {
      ...
      const grpLabel =
        isEventsMode && colKey === "mat" && firstRow?.nome
          ? `Funcionário: ${label} - ${firstRow.nome}`
    ...
    switch (colId) {
      ...
      case "saldoProximoBH": {
        ...
        return (
          <td
            key={colId}
            className={`hdm-td-num ${v != null && v < 0 ? "hdm-c-ause" : "hdm-c-pres"}`}
          >
  ...
  const evtTotalFor = (colId, t, labelText = null, labelStyle = null) => {
    ...
    switch (colId) {
      ...
      case "hrsPlan":
        return (
          <td key={colId} className="hdm-td-num" title={evtTotalsFootnote || undefined}>
  ...
  const shell = (
    <>
      <div
        className={embedded ? "hdm-embedded" : "hdm-overlay"}
        data-theme={theme}
      ...
      >
        <div className="hdm-modal" style={modalStyle} role="dialog" aria-modal={!embedded}>
          {!embedded ? (
            <div className="hdm-header" onMouseDown={onDragStart}>
              <span className="hdm-icon">{isEventsMode ? "📋" : "📅"}</span>
              <div className="hdm-title-block">
                <span className="hdm-title">Colaboradores</span>
          ...
          {isEventsMode && !isPosEmbedded && (
            <div className="hdm-pills-row">
              ...
              {pillFilter && (
                <button
                  type="button"
                  className="hdm-pill hdm-pill-clear"
                ...
                <React.Fragment key={colId}>
                  ...
                  <span className="hdm-group-chip">
                    ...
                    <button
                      ...
                      disabled={groupBusy}
                      onClick={() => applyGroupBy((prev) => prev.filter((k) => k !== colId))}
                    >
                      ×
                    </button>
                  </span>
          ...
          >
            ...
            <table className="hdm-table" style={{ width: modalTableWidth, minWidth: "100%" }}>
              <colgroup>
                ...
                {isApiMode && apiData.useGroupList && !apiData.isLoading ? (
                  ...
                  <>
                    {flatRowIndices
                      ...
                      .map((rowIdx, i) => renderLeafRow(tableDataRows[rowIdx], rowIdx, `${i}-`))}
              ...
              {filteredCount > 0 && !hideGridTotals && (
                <tfoot>
          ...
          {!embedded ? <div className="hdm-resize-handle" onMouseDown={onResizeStart} /> : null}
        </div>
      </div>
      {calcEv && (
        <HorasCalcModal
          ...
          >
            ...
            <div className="hdm-fp-cond-row">
              <select
                ...
                onChange={(e) => setFilterCondDraft((prev) => ({ ...prev, op: e.target.value }))}
              ...
              <input
                ...
                onChange={(e) => setFilterCondDraft((prev) => ({ ...prev, value: e.target.value }))}
              />
            </div>
                  ...
                  <>
                    ...
                    {!filterPopValuesPending && all.length === 0 ? (
                      <div className="hdm-fp-item hdm-fp-note">
                        Nenhum valor distinto nesta coluna.
                      </div>
                    ) : null}
