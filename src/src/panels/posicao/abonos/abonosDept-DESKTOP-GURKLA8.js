...
function rebuildAbonosStorageState(pendentesRows, efetuadosRows, meta = {}) {
  ...
  return {
    ...
    fileNamePendentes:
      meta.kind === ABONOS_KIND.efetuados ? meta.fileNamePendentes || "" : meta.fileName || "",
    fileNameEfetuados:
      meta.kind === ABONOS_KIND.efetuados ? meta.fileName || "" : meta.fileNameEfetuados || "",
...
export function applyAbonosSheetImport(newRows, meta = {}, kind = ABONOS_KIND.pendentes) {
  const existing =
    (meta.existingStored && typeof meta.existingStored === "object"
      ? meta.existingStored
      : loadKpiAbonos()) || {};
  const pend =
    kind === ABONOS_KIND.pendentes ? newRows : getAbonosDetailRows(existing, ABONOS_KIND.pendentes);
  const efet =
    kind === ABONOS_KIND.efetuados ? newRows : getAbonosDetailRows(existing, ABONOS_KIND.efetuados);
  return rebuildAbonosStorageState(pend, efet, {
    ...
    fileNamePendentes:
      kind === ABONOS_KIND.pendentes
        ? meta.fileName || ""
        : existing.fileNamePendentes || existing.fileName || "",
    fileNameEfetuados:
...
export function abonosDetailRowsToEvents(detailRows, opts = {}) {
  ...
  return normalizeAbonosDetailRows(detailRows)
    ...
    .map((row) => ({
      ...
      horario: "",
      marcacao: "",
      _cat: kind === ABONOS_KIND.efetuados ? "justificadas" : "pendente",
...
export function diagnoseAbonosPendentesSheet(aoa) {
  ...
  if (headerIndex < 0 || missingCols.length) {
    issues.push("Nenhuma linha com as colunas obrigatórias: Departamento, Matrícula, Nome e Data.");
    if (headerIndex >= 0 && missingCols.length) {
...
export function formatAbonosImportSummary(parsed, kind = ABONOS_KIND.pendentes) {
  ...
  const n = isEfetuados
    ? Number(parsed?.countEfetuados) || 0
    : Number(parsed?.countPendentes ?? parsed?.count) || 0;
  if (!n)
    return isEfetuados ? "Nenhum abono efetuado importado." : "Nenhum abono pendente importado.";
  ...
  const fileName = isEfetuados
    ? parsed?.fileNameEfetuados
    : parsed?.fileNamePendentes || parsed?.fileName;
...
export function parseAbonosCsv(csvText) {
  ...
  const header = lines[0].split(delim).map((s) =>
    String(s || "")
      .trim()
      .toLowerCase(),
  );
...
export function buildAbonosByDept(dia, opts = {}) {
  ...
  const allRows = Array.from(map.values())
    .map((r) => ({
      ...r,
      sla: calcAbonosSla(r.pendentes, r.efetuados, r.sla),
    }))
...
export function buildAbonosDeptColaboradores(histRows = [], dept = "", stored = null) {
  ...
  if (imported.length) {
    ...
    for (const row of imported) {
      ...
      const eventName =
        ...
        ).trim() || (kind === ABONOS_KIND.efetuados ? "Abono efetuado" : "Abono pendente");
  ...
  (Array.isArray(histRows) ? histRows : []).forEach((histRow) => {
    ...
    (Array.isArray(histRow?._events) ? histRow._events : []).forEach((ev) => {
      if (String(ev?._cat || "").toLowerCase() !== "ausentes") return;
      const evDept = String(
        ev?.depto_desc || ev?.depto || ev?.departamento || ev?.departamento_desc || "",
      ).trim();
