...
test("buildAbonosByDept nao reporta SLA 100 quando nao ha dados", () => {
  const { rows, totals } = buildAbonosByDept(null, { histRows: [], stored: null });
  assert.deepEqual(rows, []);
  assert.equal(totals.pendentes, 0);
  assert.equal(totals.efetuados, 0);
  assert.equal(totals.sla, null);
});

test("abonos pendentes viram eventos pendentes sem contaminar ausentes", () => {
  const events = abonosDetailRowsToEvents(
    [{ departamento: "RH", matricula: "1", nome: "A", data: "2026-06-01", horasMin: 60 }],
    { kind: ABONOS_KIND.pendentes },
  );
  assert.equal(events.length, 1);
  assert.equal(events[0]._cat, "pendente");
  assert.equal(events[0].abonoPendente, true);
});
