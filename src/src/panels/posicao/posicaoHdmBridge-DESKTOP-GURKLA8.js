...
import { ABONOS_KIND, abonosDetailRowsToEvents, getAbonosDetailRows } from "./abonos/abonosDept.js";
...
function periodFieldsFromColab(c) {
  ...
  return {
    inicio: getGridCellText(c, "inicio"),
    termino: getGridCellText(c, "termino"),
...
function colabKey(c) {
  ...
  const nome = String(c?.nome || c?.colaborador || "")
    .trim()
    .toLowerCase();
...
export function filterEventsForPosListKey(events, posKey, eventCategories = []) {
  ...
  if (k === "abonos_pendentes")
    return list.filter((ev) => ev?._cat === "ausentes" || ev?.abonoPendente);
  ...
  if (k === "folga" || k === "ferias" || k === "afastados")
    return list.filter((ev) => ev?._cat === "justificadas");
...
export function buildPosListModalData({
  ...
  const matches = (c) => {
    ...
    if (deptoFilter && dep !== deptoFilter) return false;
    return true;
  };
