...
import { areManualImportsEnabled, getPosicaoDataPolicy } from "./api/posicaoDataPolicy.js";
...
export function PbConfigImportacoesTab({
  ...
  const dataPolicy = getPosicaoDataPolicy();
  const manualImportsEnabled = areManualImportsEnabled();
  const canImport = (key) => !permissions || permissions[key] !== false;
  const importPermissionKeys = [
    "xlsx",
    "eventos",
    "tabela",
    "bancoHoras",
  ...
  const hasAnyImportPermission = importPermissionKeys.some((key) => canImport(key));
  ...
  if (!manualImportsEnabled) {
    return (
      <div className="pb-cfg-tab-panel">
        <div className="pb-cfg-provisional-banner" role="note" aria-label="Modo API ativo">
          <div className="pb-cfg-provisional-banner-head">
            <span className="pb-cfg-provisional-badge">API</span>
            <strong>Importações manuais desativadas</strong>
          </div>
          <p>
            A fonte de dados da Posição está configurada como <strong>{dataPolicy.source}</strong>.
            Neste modo, planilhas deixam de alimentar o painel e os dados devem vir da API oficial.
          </p>
          <ul>
            <li>Use os endpoints da integração .NET para posição, absenteísmo e indicadores.</li>
            <li>Metas, categorias de horas e premissas continuam disponíveis nas demais abas.</li>
          </ul>
        </div>
        ...
        <PeriodoApuracaoBlock
          ...
          hint="Intervalo usado pelos contratos da API e pelos filtros do painel."
        />
      </div>
    );
  }
  ...
  return (
    <div className="pb-cfg-tab-panel">
      ...
      {!hasAnyImportPermission && (
        <div className="pb-cfg-import-panel">
          <span className="pb-cfg-label">Sem importacoes liberadas para este perfil</span>
          <span className="pb-cfg-hint">
            Solicite ao administrador as permissoes necessarias para alimentar este painel
            manualmente.
          </span>
        </div>
      )}
