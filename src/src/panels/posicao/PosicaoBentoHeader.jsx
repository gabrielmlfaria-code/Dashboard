...
export function PosicaoBentoHeader({
  ...
  const operationalDiagnosisData = useMemo(
    () => ({
      ...
      resumo: {
        ...
        forcaPrevista: Number(m.quadroPrevista ?? m.prevista) || 0,
        vagas: Number(m.quadroVagas ?? m.vagas) || 0,
      },
      departamentos: (Array.isArray(deptStats) ? deptStats : []).map((dept) => ({
        id: dept.id ?? dept.idDepartamento ?? dept.depto,
        idDepartamento: dept.id ?? dept.idDepartamento ?? null,
  ...
  const financialImpact = useMemo(() => {
    const entries = Object.values(fpdMapProp || {});
    const values = (field) =>
      entries
  ...
  }, [fpdMapProp, histHorasNaoTrabalhadas, histRadar.horasExtras]);
  ...
  const resolveNlDeptoFilter = useCallback(
    (value) => {
      ...
      const normalized = opts.map((d) =>
        d && typeof d === "object" && d.value != null
          ? { value: String(d.value), label: String(d.label || d.value) }
          : { value: String(d), label: String(d) },
      );
      const exact = normalized.find((d) => d.value === val || d.label === val);
      if (exact) return exact.value;
      const low = val.toLowerCase();
      const partial = normalized.find(
        (d) =>
          d.label.toLowerCase() === low ||
          d.label.toLowerCase().includes(low) ||
          low.includes(d.label.toLowerCase()),
      );
      return partial?.value || val;
    },
    [deptoOptions],
  );
...
<table><thead><tr>${head}</tr></thead><tbody>${body}</tbody></table>
  ...
  // ===== Força Prevista por Departamento (idDepartamento) =====
  const [fpdOpen, setFpdOpen] = useState(false);
  const [fpdMap, setFpdMap] = useState(() => ({ ...(fpdMapProp || {}) }));
  const [fpdSearch, setFpdSearch] = useState("");
  const [fpdOnlyEmpty, setFpdOnlyEmpty] = useState(false);
  const [fpdMoneyDraft, setFpdMoneyDraft] = useState({});
  const fpdMoneyKey = (deptoId, field) => `${deptoId}|${field}`;

  const fpdCatalog = useMemo(() => {
    const m = new Map();
    (Array.isArray(fpdList) ? fpdList : []).forEach((d) => {
      const id = String(d.id ?? d.idDepartamento ?? "");
      if (!id) return;
      m.set(id, {
    ...
    (Array.isArray(departamentosApi) ? departamentosApi : []).forEach((d) => {
      const id = String(d.id ?? d.idDepartamento ?? "");
      if (!id) return;
      if (!m.has(id)) {
        m.set(id, {
          idDepartamento: Number(id),
          nome: d.nome || id,
          ativos: d.ativos ?? d.forcaAtual ?? 0,
        });
      }
    });
    (deptoOptions || []).forEach((opt) => {
      if (opt && typeof opt === "object" && opt.value != null) {
        const id = String(opt.value);
        if (!m.has(id)) {
          m.set(id, {
            idDepartamento: /^\d+$/.test(id) ? Number(id) : id,
            nome: opt.label || id,
            ativos: 0,
          });
        }
    ...
    return m;
  }, [fpdList, departamentosApi, deptoOptions]);

  useEffect(() => {
    if (fpdOpen) {
      setFpdMap({ ...(fpdMapProp || {}) });
      setFpdSearch("");
  ...
  const fpdAtivos = useMemo(() => {
    const m = {};
    (deptStats || []).forEach((r) => {
      const k = String(r?.id ?? r?.idDepartamento ?? "");
      if (!k) return;
      const total = [
        "presentes",
        "falta",
  ...
  const fpdRows = useMemo(() => {
    const set = new Set(fpdCatalog.keys());
    Object.keys(fpdMap || {}).forEach((k) => set.add(String(k)));
    Object.keys(fpdAtivos || {}).forEach((k) => set.add(String(k)));
    let rows = Array.from(set);
    rows.sort((a, b) => {
      const na = fpdCatalog.get(a)?.nome || a;
      const nb = fpdCatalog.get(b)?.nome || b;
      return na.localeCompare(nb, "pt-BR", { sensitivity: "base" });
    });
    const q = (fpdSearch || "").toLowerCase().trim();
    if (q) {
      rows = rows.filter((id) => {
        const nome = fpdCatalog.get(id)?.nome || id;
        return nome.toLowerCase().includes(q) || String(id).includes(q);
      });
    }
    if (fpdOnlyEmpty) rows = rows.filter((id) => !getForcaPrevistaQty(fpdMap?.[id]));
    return rows;
  }, [fpdCatalog, fpdMap, fpdAtivos, fpdSearch, fpdOnlyEmpty]);
  ...
  const fpdTotalDeptos = useMemo(() => {
    ...
    return set.size;
  }, [fpdCatalog, fpdAtivos]);

  const ensureFpdEntry = (deptoId, prev = {}) => {
    const id = String(deptoId);
    const cat = fpdCatalog.get(id);
    return {
      idDepartamento: Number(cat?.idDepartamento ?? (/^\d+$/.test(id) ? id : NaN)) || undefined,
      nome: cat?.nome || prev?.nome || id,
      prevista: null,
      custoHora: null,
      custoHExtra: null,
      ...prev,
  ...
  const handleFpdChange = (deptoId, field, raw) => {
    const id = String(deptoId);
    setFpdMap((prev) => {
      const next = { ...(prev || {}) };
      const cur = ensureFpdEntry(id, next[id] || {});
      if (field === "prevista") {
      ...
      if (hasData) next[id] = cur;
      else delete next[id];
      return next;
    });
  };
  const getFpdMoneyValue = (deptoId, field, stored) => {
    const key = fpdMoneyKey(deptoId, field);
    if (Object.prototype.hasOwnProperty.call(fpdMoneyDraft, key)) return fpdMoneyDraft[key];
    return formatFpdMoneyDisplay(stored);
  };
  const handleFpdMoneyChange = (deptoId, field, raw) => {
    setFpdMoneyDraft((prev) => ({ ...prev, [fpdMoneyKey(deptoId, field)]: raw }));
  };
  const commitFpdMoney = (deptoId, field) => {
    const key = fpdMoneyKey(deptoId, field);
    const raw = Object.prototype.hasOwnProperty.call(fpdMoneyDraft, key)
      ? fpdMoneyDraft[key]
      : formatFpdMoneyDisplay(fpdMap?.[deptoId]?.[field]);
    setFpdMoneyDraft((prev) => {
      const next = { ...prev };
      delete next[key];
      return next;
    });
    handleFpdChange(deptoId, field, raw ?? "");
  };
  const applyFpdDrafts = (base) => {
    ...
    Object.entries(fpdMoneyDraft).forEach(([key, raw]) => {
      ...
      if (sep <= 0) return;
      const deptoId = key.slice(0, sep);
      const field = key.slice(sep + 1);
      if (field !== "custoHora" && field !== "custoHExtra") return;
      const cur = ensureFpdEntry(deptoId, next[deptoId] || {});
      const trimmed = String(raw ?? "").trim();
      cur[field] = trimmed === "" ? null : parseFpdMoney(trimmed);
      const hasData = cur.prevista || cur.custoHora != null || cur.custoHExtra != null;
      if (hasData) next[deptoId] = cur;
      else delete next[deptoId];
    });
    return next;
  };
  const handleFpdUseAtivos = (deptoId) => {
    const id = String(deptoId);
    const a = Number(fpdAtivos?.[id]) || 0;
    if (a > 0) {
      setFpdMap((prev) => ({
        ...(prev || {}),
        [id]: ensureFpdEntry(id, { prevista: a, ...(prev?.[id] || {}) }),
      }));
    }
  };
    ...
    setFpdMap((prev) => {
      ...
      Object.entries(fpdAtivos || {}).forEach(([k, v]) => {
        ...
        if (n > 0 && !getForcaPrevistaQty(next[k])) {
          next[k] = ensureFpdEntry(k, { prevista: n, ...(next[k] || {}) });
        }
      });
      return next;
    });
  ...
  const handleFpdSave = async () => {
    ...
    setFpdMap(merged);
    setFpdMoneyDraft({});
    if (onSaveFpd) await onSaveFpd(merged);
    setFpdOpen(false);
  };
  const handleFpdClearDepto = async (deptoId) => {
    const id = String(deptoId);
    setFpdMap((prev) => {
      const next = { ...(prev || {}) };
      delete next[id];
    ...
    if (onExcluirFpdDepto && /^\d+$/.test(id)) {
      try {
        await onExcluirFpdDepto(Number(id));
      } catch {
            ...
            >
              ...
              <div className="pb-fpd-toolbar">
                <input
                  ...
                  value={fpdSearch}
                  onChange={(e) => setFpdSearch(e.target.value)}
                />
              ...
              <div className="pb-fpd-list">
                ...
                {fpdRows.length === 0 ? (
                  ...
                  fpdRows.map((deptoId) => {
                    const ativos = Number(fpdAtivos?.[deptoId]) || 0;
                    const entry = fpdMap?.[deptoId] || {};
                    const nome = fpdCatalog.get(deptoId)?.nome || entry.nome || deptoId;
                    const prevista = getForcaPrevistaQty(entry);
                    const has =
                      prevista > 0 || entry.custoHora != null || entry.custoHExtra != null;
                    return (
                      <div className={`pb-fpd-row${has ? " has" : ""}`} key={deptoId}>
                        <span className="pb-fpd-name" title={nome}>
                          {nome}
                        </span>
                        ...
                        <input
                          ...
                          onChange={(e) => handleFpdChange(deptoId, "prevista", e.target.value)}
                        />
                        <input
                          ...
                          inputMode="decimal"
                          placeholder="R$ 0,00"
                          value={getFpdMoneyValue(deptoId, "custoHora", entry.custoHora)}
                          onChange={(e) => handleFpdMoneyChange(deptoId, "custoHora", e.target.value)}
                          onBlur={() => commitFpdMoney(deptoId, "custoHora")}
                        />
                        <input
                          className="pb-cfg-input pb-fpd-input pb-fpd-input-money"
                          type="text"
                          inputMode="decimal"
                          placeholder="R$ 0,00"
                          value={getFpdMoneyValue(deptoId, "custoHExtra", entry.custoHExtra)}
                          onChange={(e) => handleFpdMoneyChange(deptoId, "custoHExtra", e.target.value)}
                          onBlur={() => commitFpdMoney(deptoId, "custoHExtra")}
                        />
                        <div className="pb-fpd-actions">
                          <button
                            ...
                            disabled={ativos <= 0}
                            title="Usar ativos"
                            onClick={() => handleFpdUseAtivos(deptoId)}
                          >
                            usar ativos
                          </button>
                          <button
                            ...
                            disabled={!has}
                            title="Limpar"
                            onClick={() => handleFpdClearDepto(deptoId)}
                          >
                            ×
                          </button>
                        </div>
            ...
            <section className="pb-side-panel pb-side-panel-quadro">
              ...
              <table className="pb-side-table">
                ...
                <tbody>
                  <tr>
                    <td>Força atual</td>
                    <td className="num c-blue">{fmt(m.quadroAtual ?? m.atual)}</td>
                  </tr>
                  {(m.quadroPrevista ?? m.prevista) != null &&
                    Number(m.quadroPrevista ?? m.prevista) > 0 && (
                    <tr>
                      <td>Força prevista</td>
                      <td className="num">{fmt(m.quadroPrevista ?? m.prevista)}</td>
                    </tr>
                  )}
                  {(m.quadroVagas ?? m.vagas) != null && (
                    <tr>
                      <td>Vagas</td>
                      <td className="num">
                        {Number(m.quadroVagas ?? m.vagas) === 0 ? (
                          <span className="c-green" title="Quadro completo">
                            0 ✓
                          </span>
                        ) : (
                          <span className="c-orange">{fmt(m.quadroVagas ?? m.vagas)}</span>
                        )}
                      </td>
                    </tr>
                  )}
