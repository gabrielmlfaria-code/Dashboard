...
export function toAbsenteeismSummaryDto({
  ...
  return {
    ...
    horasPlanejadasMinutos: result.plannedMinutes,
    horasTrabalhadasMinutos: toNumber(workedMinutes),
    horasAusentesMinutos: result.unjustifiedAbsentMinutes,
    horasJustificadasMinutos: result.justifiedAbsentMinutes,
    indicePercentual: result.pct,
    metaPercentual: result.metaPct,
    calculo: {
      formula: "(horasAusentesMinutos / horasPlanejadasMinutos) * 100",
      base: "horasPlanejadasMinutos",
      entradas: {
        horasAusentesMinutos: result.unjustifiedAbsentMinutes + result.justifiedAbsentMinutes,
        horasPlanejadasMinutos: result.plannedMinutes,
      },
      resultado: result.pct,
    },
...
export function toBankHoursDto({ periodo, summary = {}, departamentos = [] } = {}) {
  ...
  return {
    periodo: normalizePeriodo(periodo),
    saldoAnteriorMinutos: totals.previousBalanceMinutes,
    creditoMinutos: totals.creditMinutes,
    debitoMinutos: totals.debitMinutes,
    saldoProximoMinutos: totals.nextBalanceMinutes,
    ...
    departamentos: departamentos.map((dept) => {
      ...
      return {
        ...
        saldoAnteriorMinutos: deptTotals.previousBalanceMinutes,
        creditoMinutos: deptTotals.creditMinutes,
        debitoMinutos: deptTotals.debitMinutes,
        saldoProximoMinutos: deptTotals.nextBalanceMinutes,
...
export function toLaborRadarDto({ periodo, ocorrencias = [], principalEvento = null } = {}) {
  ...
  const totalOcorrencias = rows.reduce(
    (sum, row) => sum + toNumber(row.ocorrencias ?? row.count, 0),
    0,
  );
  ...
  const principal =
    ...
    rows
      .slice()
      .sort(
        (a, b) => toNumber(b.ocorrencias ?? b.count, 0) - toNumber(a.ocorrencias ?? a.count, 0),
      )[0];
