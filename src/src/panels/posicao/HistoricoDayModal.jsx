...
function getColFilterValue(row, col, { stackHrsMrc = false, isEventsMode = false } = {}) {
  ...
  if (col === "data") return eventDateKey(row) || "";
  if (col === "inicio" || col === "termino") {
    const text = getGridCellText(row, col);
    return normDateKey(text) || text || "";
  }
  const raw = row[col];
  if (raw == null || raw === "") return "";
  return String(raw);
  ...
  const isEventsMode =
    ...
    (embedded &&
      (embeddedPosListKey === "banco_horas" ||
        ...
        posListUsesPeriodoCols(embeddedPosListKey)));
