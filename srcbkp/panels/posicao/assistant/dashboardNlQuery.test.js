...
describe("getNlChipsForSurface", () => {
  it("absenteísmo inclui chips de cards e risco do painel", () => {
    const ids = getNlChipsForSurface("absenteismo").map((c) => c.id);
...
describe("linguagem natural para heatmap", () => {
  ...
  it("abre heatmap filtrado por departamento", () => {
    ...
    assert.equal(matchDashboardNlIntent("mapa de calor do departamento producao"), "heatmap_filtro");
...
describe("getNlChipGroupsForSurface", () => {
  it("agrupa chips de absenteísmo em cards temáticos", () => {
    ...
    assert.equal(faltas.chips.some((c) => c.id === "faltas_concentracao"), true);
    const cards = groups.find((g) => g.id === "cards");
    assert.equal(cards.chips.some((c) => c.id === "banco_horas_saldo"), true);
  });

  it("radar expõe risco e pessoas", () => {
    const groups = getNlChipGroupsForSurface("radar");
    assert.equal(groups.some((g) => g.id === "risco"), true);
    assert.equal(groups.some((g) => g.id === "pessoas"), true);
    assert.equal(groups.some((g) => g.id === "faltas"), false);
  });
});
...
describe("matchDashboardNlIntent — novos cards", () => {
  ...
  it("detecta art. 473 antes de saúde preventiva genérica", () => {
    assert.equal(matchDashboardNlIntent("Há ausências do art. 473 no período?"), "art_473_ausencias");
  ...
  it("detecta horas planejadas e ranking", () => {
    assert.equal(matchDashboardNlIntent("Quantas horas planejadas no período?"), "horas_planejadas");
    assert.equal(matchDashboardNlIntent("Qual o ranking de departamentos?"), "ranking_departamentos");
  });
});
