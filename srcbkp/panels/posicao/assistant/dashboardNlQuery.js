...
/** Minutos → texto legível (ex.: 5.589 h ou 45 min). */
export function fmtMinutesReadable(min) {
...
function findEntityInQuestion(question, values) {
  ...
  return [...(values || [])]
    .map((item) => {
      const label = typeof item === "string" ?item : item?.nome || item?.dept || item?.label || "";
...
function parseHeatmapPeriod(question) {
  ...
  const dates = [...String(question || "").matchAll(/(\d{1,2})[\/.-](\d{1,2})[\/.-](\d{2,4})/g)].map((m) => {
...
const INTENTS = [
  {
    ...
    test: (q) =>
      /(colaborador|funcionario|empregado).*(mais falt|mais ausen|maior ausen|mais ocorren)/.test(q) ||
  ...
  },
  {
    ...
    test: (q) =>
      /(custo|valor|impacto financeiro|quanto custa|r\$).*(absente|falta|ausen|hora perdida)/.test(q) ||
  ...
  {
    id: "justificadas_mix",
    test: (q) => /justificad|atestado|abono|licenca/.test(q) && /(injust|falta|ausen|horas)/.test(q),
  },
  {
    id: "presenca_pct",
  ...
  },
  {
    ...
    test: (q) =>
      /horas/.test(q) &&
      /ausen/.test(q) &&
      !/injust|justific|planej|extra|perdida|trabalh/.test(q),
  },
  {
    id: "perda_percentual",
    test: (q) =>
  ...
  },
  {
    ...
    test: (q) =>
      ...
      /(segunda|terca|quarta|quinta|sexta|sabado|domingo).*(critico|falta|ausen|absenteismo)/.test(q),
  },
  {
    id: "insights_periodo",
    test: (q) =>
...
function deptOrColabRanking(analise, total) {
  if (analise.topDeptos.length > 0) return { title: "Por departamento", rows: analise.rankingDeptos };
  return {
...
function answerFaltasConcentracao(ctx, q) {
  ...
  if (!total) {
    return {
      ...
      text:
        "Não há eventos classificados como Ausentes no período filtrado. Verifique a importação ou categorize os eventos em Configurações › Horas.",
...
function answerAbsIndice(ctx) {
  ...
  if (t.absDelta != null && Math.abs(t.absDelta) >= 0.1) {
    ...
    lines.push(`Na comparação interna do período (1ª vs 2ª metade), o índice **${dir} ${pct(Math.abs(t.absDelta))}** pontos.`);
  }
...
function answerDiaCritico(ctx) {
  ...
  const pat = ctx.patternDow
    ?` Há concentração em **${ctx.patternDow}s**.`
    : "";
  return {
    intent: "dia_critico",
...
function answerRankingDepartamentos(ctx) {
  ...
  return {
    ...
    structured: {
      ...
      ranking,
      tips: ["O filtro aplica o departamento selecionado na barra superior e abre a tabela histórica."],
    },
...
function answerPadraoSemana(ctx) {
  ...
  const patTxt = pat
    ?`Há concentração em **${pat}s** entre os dias mais críticos.`
    : "";
  const critTxt = crit
...
function answerArt473Ausencias(ctx) {
  ...
  const alertaTxt = a.alertas > 0 ? ` **${fmtNum(a.alertas)}** colaborador(es) acima do limite de 3 dias/12 meses.` : "";
  const semComTxt =
    a.semComunicacao > 0
      ? ` **${fmtNum(a.semComunicacao)}** ocorrência(s) sem comunicação prévia registrada.`
      : " Todas com comunicação prévia registrada.";
  const calOk = a.calendarioOk > 0 ? ` Calendário: **${fmtNum(a.calendarioOk)}** campanha(s) concluída(s).` : "";
  return {
    intent: "art_473_ausencias",
...
function answerAbonosPendentes(ctx) {
  ...
  const topTxt = top ?` Maior fila: **${top.dept}** (**${fmtNum(top.pendentes)}** pendentes).` : "";
  return {
    intent: "abonos_pendentes",
...
function answerHeatmapFiltro(question, ctx) {
  ...
  if (!value) {
    return {
      ...
      text:
        "Consigo criar o heatmap, mas não localizei esse colaborador/departamento no histórico do período atual. Tente informar o nome como está na planilha ou filtre o período antes.",
      empty: true,
  ...
  return {
    ...
    action: openFilterAction("open_radar_heatmap", field, field === "departamento" ? "Departamento" : "Colaborador", value, {
      period,
    }),
  };
}
...
function answerFallback(ctx, surface) {
  const chips = getNlChipsForSurface(surface).slice(0, 6);
  ...
  return {
    ...
    structured: {
      ...
      subheadline: "Use uma das sugestões ou pergunte sobre os indicadores disponíveis no dashboard.",
      tips: [
