...
export function MensalListCard({
  data = null,
  onOpenEvent = null,
  periodoApuracao = null,
  fmtHMMilhar = defaultFmtHMMilhar,
  style,
}) {
  ...
  const mensalRows = useMemo(
    () => (hasData ? sortMensalRows(data.rows || []) : []),
    [data?.rows, eventSortDir, hasData, monthlyCategoryConfig],
  );
  ...
  const consultaRows = useMemo(
    () =>
      hasData
        ...
        : [],
    [data?.rows, eventSortDir, hasData, monthlyCategoryConfig, months, selectedSet],
  );
  ...
  const eventsGridRows = visibleRows.reduce((acc, row) => {
    const label = splitEventLabel(row.event);
    ...
    acc.push({
      ...
      code: label.code,
      showCategory: previous?.categoryKey !== category.key,
  ...
  return (
    <div
      className={`pb-cell pb-mensal ${expanded ? "pb-mensal--expanded" : ""}`}
      style={style}
                    ...
                    >
                      {viewMode === "consulta" ? (
                        ...
                        <span role="cell" className="pb-mensal-period-event" title={row.event}>
                          <button
                            type="button"
                            className="pb-mensal-event-open-btn"
