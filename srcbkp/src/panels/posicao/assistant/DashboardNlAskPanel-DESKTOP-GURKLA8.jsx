...
import { DashboardNlToggleButton } from "./DashboardNlToggleButton.jsx";
...
function clampModalSize(w, h) {
  const maxH = Math.min(
    DNL_SIZE_MAX.height,
    typeof window !== "undefined" ? window.innerHeight - 24 : DNL_SIZE_MAX.height,
  );
  return {
      ...
      <ol className="dnl-rank-list">
        {items.map((row, i) => {
          ...
          return (
            <li
              key={`${row.name}-${i}`}
              className={`dnl-rank-row${clickable ? " is-clickable" : ""}`}
            >
              <button
                type="button"
  ...
  return (
    <ul className="dnl-insight-metrics" aria-label={ariaLabel}>
      {metrics.map((m, i) => (
        <li
          key={`${m.label}-${i}`}
          className={`dnl-insight-metric${clickable ? " is-clickable" : ""}`}
        >
          <button
            type="button"
...
const NL_RADAR_ACTIONS = new Set([
  "open_radar",
  "open_radar_eventos",
  "open_radar_heatmap",
  "open_radar_passivo",
...
function getRecommendedNlChips(context, chipGroups) {
  ...
  return ids
    .slice(0, 3)
    .map((id) => findChipById(chipGroups, id))
    .filter(Boolean);
}
    ...
    >
      ...
      <div className="dnl-insight-body">
        {mode === "structured" ? (
          <>
            <div
              className={`dnl-insight-hero${display.variant === "generic_only" ? " is-warn" : ""}`}
            >
      ...
      <footer className="dnl-insight-footer">
        ...
        {!disclaimerSeen ? (
          <p className="dnl-insight-disclaimer">
            Resposta calculada com os filtros e o período visíveis no dashboard · não substitui
            auditoria ou parecer jurídico
          </p>
        ) : (
          <button
            type="button"
...
export function DashboardNlAskPanel({
  ...
  const copyAnswer = useCallback(() => {
    ...
    copyTextToClipboard(text).then((ok) => {
      if (!ok) {
        setSpeechError(
          "Não foi possível copiar automaticamente. Selecione o texto da resposta para copiar.",
        );
        return;
      }
      setCopied(true);
    ...
    (q, forceIntent) => {
      ...
      startTransition(() => {
        ...
        setHistory((prev) => {
          ...
          const next = [entry, ...prev.filter((h) => h.question !== text)].slice(
            0,
            DNL_HISTORY_MAX,
          );
          saveHistory(next);
  ...
  const chipGroups = useMemo(() => getNlChipGroupsForSurface(surface, context), [surface, context]);
  ...
  const activeGroup = chipGroups.find((g) => g.id === activeSuggestGroup) || chipGroups[0] || null;
      ...
      >
        <div
          ...
          style={{
            ...
            transform:
              dragPos.x || dragPos.y ? `translate(${dragPos.x}px, ${dragPos.y}px)` : undefined,
          }}
          onMouseDown={(e) => e.stopPropagation()}
        >
          <header className="dnl-modal-head" onMouseDown={onDragStart} title="Arraste para mover">
  ...
  return (
    <>
      <div
        ...
        data-theme={theme}
      >
        <DashboardNlToggleButton ref={toggleButtonRef} modalOpen={modalOpen} onOpen={openModal} />
      </div>
      {modal}
    </>
  );
