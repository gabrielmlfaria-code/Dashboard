...
describe("posicaoLocalAdapters", () => {
  ...
  it("maps absenteeism using categorized absence minutes", () => {
    ...
    assert.equal(dto.indicePercentual, 16.7);
    assert.equal(dto.horasTrabalhadasMinutos, 42067);
    assert.equal(dto.horasAusentesMinutos, 16065);
    assert.equal(dto.calculo.base, "horasPlanejadasMinutos");
  ...
  it("maps bank hours preserving imported previous balance", () => {
    ...
    assert.equal(dto.saldoAnteriorMinutos, 2223);
    assert.equal(dto.saldoProximoMinutos, 3659);
    assert.equal(dto.departamentos[0].saldoAnteriorMinutos, 2223);
