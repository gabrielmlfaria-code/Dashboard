...
import { getBancoHorasImportRows, parseBancoHorasDate } from "./banco-horas/bancoHoras.js";
...
const buildBancoHorasModalEvents = (depto, fallbackDate = "") => {
  ...
  return items.map((row, idx) => {
    ...
    const data = parseBancoHorasModalDate(
      row?.periodoInicial || row?.data || row?.date,
      fallbackDate,
    );
    ...
    return {
      mat:
        bancoHorasModalLabel(row, ["matricula", "colaborador.matricula", "codigo", "cod"]) ||
        `${idx + 1}`,
      nome: bancoHorasModalLabel(row, ["nome", "colaborador.nome", "colaborador", "name"]),
      genero: bancoHorasModalLabel(row, ["genero", "sexo"]),
      data,
      depto:
        bancoHorasModalLabel(row, ["departamento", "departamentoNome", "depto", "depto_desc"]) ||
        deptName,
...
const filterBancoHorasEventsByKpi = (events, kpi) => {
  ...
  switch (kpi) {
    ...
    case "movimento":
      return list.filter((ev) => Number(ev?.creditoBH || 0) > 0 || Number(ev?.debitoBH || 0) > 0);
    case "saldo_anterior":
...
const filterBancoHorasRowsByKpi = (rows, kpi) => {
  ...
  switch (kpi) {
    ...
    case "movimento":
      return list.filter((row) => Number(row?.credito || 0) > 0 || Number(row?.debito || 0) > 0);
    case "saldo_anterior":
      return list.filter(
        (row) =>
...
export function PosicaoDiaPanel() {
  ...
  const showApiError = !showLoading && !!err && !dia;
  ...
  const resolvePosListModalDates = useCallback(
    (refDate = "") => {
      ...
      const { dateFrom, dateTo } = resolveModalDatesFromApuracao(
        periodoApuracao,
        histRowsForPeriodo,
      );
      return {
  ...
  const normalizeOperationalListKey = (raw) => {
    const value = String(raw || "")
      ...
      .replace(/[\u0300-\u036f]/g, "")
      .toLowerCase();
    if (["falta", "faltas", "ausente", "ausentes", "absence"].includes(value)) return "faltas";
    if (["atraso", "atrasos", "delay"].includes(value)) return "atrasos";
  ...
  const openDeptModal = (options = null) => {
    ...
    if (hasOperationalPayload) {
      const ref = String(
        presentesDate || dia?.data_referencia || new Date().toISOString().slice(0, 10),
      );
  ...
  const openMensalEventColaboradores = (row) => {
    ...
    const monthFrom = row?.month
      ? ymToIsoStart(String(row.month).replace(/^(\d{2})\/(\d{4})$/, "$2-$1"))
      : "";
    const monthTo = row?.month
      ? ymToIsoEnd(String(row.month).replace(/^(\d{2})\/(\d{4})$/, "$2-$1"))
      : "";
    ...
    async (file) => {
      ...
      try {
        ...
        const res = await importPosicaoXlsxFile(file, {
          ...
          targetDate: normDateKey(presentesDate) || new Date().toISOString().slice(0, 10),
        });
        if (!res) return;
  ...
  const handleImportPosicaoBackup = useCallback(async (file) => {
    try {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (u) {
        const upd = lastUpdText && lastUpdText !== "--:--" ? String(lastUpdText).toLowerCase() : "";
        if (String(posListKey || "") === "banco_horas") {
    ...
    (depto) => {
      ...
      if (!items.length && deptName) {
        items = getBancoHorasImportRows(null, bancoHorasHistSource).filter((row) => {
          const rowDept =
            row?.departamento || row?.departamentoNome || row?.depto || row?.depto_desc || "";
  ...
  return (
    <div className="chart-panel" id="cpPosicao" style={{ minHeight: 0, flex: 1, overflow: "auto" }}>
      ...
      <div className="conn-banner" id="pos_connBanner" style={{ display: err ? "flex" : "none" }}>
        ...
        ? createPortal(
            <div className="pos-list-hdm-wrap">
              ...
              <HistoricoDayModal
                ...
                deptFilterLabel={
                  ["banco_horas", "abonos_pendentes", "abonos_efetuados"].includes(
                    String(posListKey || ""),
                  )
                    ? posListDeptoFilter
                    : ""
