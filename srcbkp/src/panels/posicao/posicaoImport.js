...
const DIA_CATEGORY_ALIASES = {
  ...
  falta: ["falta", "faltas", "Falta", "Faltas", "faltasGrupo", "faltasRows"],
  atraso: ["atraso", "atrasos", "Atraso", "Atrasos", "atrasosGrupo", "atrasosRows"],
  folga: ["folga", "folgas", "Folga", "Folgas"],
  ferias: ["ferias", "férias", "Ferias", "Férias", "feriasRows", "feriasGrupo"],
  afastados: ["afastados", "Afastados", "afastadosGrupo"],
...
function dedupeColaboradores(list) {
  ...
  for (const c of list) {
    ...
    const key = mat ? `mat:${mat}` : nome ? `nome:${nome}` : "";
...
export function normalizePosicaoDiaPayload(raw) {
  ...
  POSICAO_CATEGORIES.forEach((canonical) => {
    ...
    colaboradores = dedupeColaboradores(colaboradores);
