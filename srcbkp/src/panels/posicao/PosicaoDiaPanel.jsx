...
import {
  ...
  forcaPrevistaMapToPutPayload,
  forcaPrevistaPatchApiList,
  buildDeptoOptionsFromForcaPrevista,
...
import { getForcaPrevistaQty } from "./posicaoSettings.js";
...
export function PosicaoDiaPanel() {
  ...
  const [deptChartFilter, setDeptChartFilter] = useState(() => {
    try {
      const raw =
        localStorage.getItem("pos_dept_chart_filter") || localStorage.getItem("pos_dept_filter");
      if (raw) {
        const arr = JSON.parse(raw);
  ...
  useEffect(() => {
    try {
      localStorage.setItem("pos_dept_chart_filter", JSON.stringify(deptChartFilter || []));
    } catch {}
  }, [deptChartFilter]);

  useLayoutEffect(() => {
    // Garantir restore do tamanho após o DOM do React existir
    try {
      ...
      if (panel && s) {
        ...
        const maxH =
          rootRect != null
            ? Math.max(200, rootRect.bottom - panelRect.top)
            : Math.max(200, window.innerHeight - panelRect.top);
        const maxW =
          rootRect != null
            ? Math.max(220, rootRect.width)
            : Math.max(220, window.innerWidth);
        let w = panelRect.width;
        let h = panelRect.height;
        if (s.h) h = Math.min(maxH, Math.max(200, parseInt(s.h, 10) || 0));
        if (s.w) w = Math.min(maxW, Math.max(220, parseInt(s.w, 10) || 0));
        if (s.w || s.h) {
  ...
  const FPD_SAVE_FILIAL_ID = 0;
  const fpdListFilialId = filialFilter ? Number(filialFilter) : 0;
  const fpdQuery = useForcaPrevistaDepartamentos({
    idFilial: fpdListFilialId,
  ...
  const patchForcaPrevistaCache = useCallback(
    (patchFn) => {
      const key = ["posicao", "forca-prevista", "departamentos", fpdListFilialId, presentesDate, 0];
      queryClient.setQueryData(key, (old) => {
        const list = Array.isArray(old) ? old : [];
        return typeof patchFn === "function" ? patchFn(list) : list;
      });
    },
    [queryClient, fpdListFilialId, presentesDate],
  );
  ...
  const handleSaveFpd = useCallback(
    async (map) => {
      patchForcaPrevistaCache((list) => forcaPrevistaPatchApiList(list, map));
      await salvarFpdMutation.mutateAsync(forcaPrevistaMapToPutPayload(FPD_SAVE_FILIAL_ID, map));
    },
    [salvarFpdMutation, patchForcaPrevistaCache],
  );

  const handleExcluirFpdDepto = useCallback(
    async (idDepartamento) => {
      patchForcaPrevistaCache((list) => forcaPrevistaClearApiItem(list, idDepartamento));
      await excluirFpdMutation.mutateAsync(idDepartamento);
    },
    [excluirFpdMutation, patchForcaPrevistaCache],
  );

  const handleLimparFpd = useCallback(async () => {
    patchForcaPrevistaCache(() => []);
    await limparFpdMutation.mutateAsync(FPD_SAVE_FILIAL_ID);
  }, [limparFpdMutation, patchForcaPrevistaCache]);
  ...
  const deptoOptions = useMemo(() => {
    ...
    const add = (value, label) => {
      ...
      if (!v) return;
      if (!byId.has(v)) byId.set(v, { value: v, label: String(label || v).trim() || v });
    };
    buildDeptoOptionsFromForcaPrevista(Array.isArray(fpdQuery.data) ? fpdQuery.data : []).forEach(
      (o) => byId.set(o.value, o),
    );
    const deptosApi = diaQuery.data?.departamentos ?? dia?.departamentos;
    (Array.isArray(deptosApi) ? deptosApi : []).forEach((d) => {
      add(d.id ?? d.idDepartamento, d.nome);
    });
    if (dia) {
      POSICAO_CATEGORIES.forEach((k) => {
        getColaboradoresFromGroup(dia?.[k]).forEach((c) => {
          const id = resolveIdDepartamento(c);
          const nome = resolveDeptoNome(c);
          if (id) add(id, nome);
          else if (nome) add(nome, nome);
        });
      });
    }
    if (Array.isArray(histTableImport)) {
      histTableImport.forEach((row) => {
        (row._employees || []).forEach((emp) => {
          const id = resolveIdDepartamento(emp);
          const nome = resolveDeptoNome(emp);
          if (id) add(id, nome);
          else if (nome) add(nome, nome);
        });
      });
  ...
  }, [dia, histTableImport, fpdQuery.data, diaQuery.data?.departamentos]);

  // Migra filtro legado por nome → id quando o mapa FPD permitir
  useEffect(() => {
    if (!deptoFilter || /^\d+$/.test(String(deptoFilter))) return;
    const mapped = fpdNomeToId[deptoFilter];
    if (mapped != null) setDeptoFilter(String(mapped));
  }, [deptoFilter, fpdNomeToId]);

  // Reseta filtro de depto se o id não existe mais nas opções
  useEffect(() => {
    if (!deptoFilter) return;
    const values = new Set(deptoOptions.map((o) => o.value));
    if (!values.has(String(deptoFilter))) setDeptoFilter("");
  ...
  const deptStats = useMemo(() => {
    ...
    const map = new Map();
    cats.forEach((k) => {
      getColaboradoresFromGroup(filteredDia?.[k]).forEach((c) => {
        const id = resolveIdDepartamento(c);
        const nome = resolveDeptoNome(c) || "—";
        const key = id ? String(id) : nome;
    ...
    return Array.from(map.values()).map((r) => {
      const atual =
        ...
        (r.entrada_prev || 0) +
        (r.nao_controla || 0);
      const fpdEntry = r.id != null ? fpdMap[r.id] : null;
      const cadastrada = fpdEntry ? getForcaPrevistaQty(fpdEntry) : 0;
  ...
  const bentoMetrics = useMemo(() => {
    ...
    // Força operacional = presentes + faltas + já saíram + entrada prevista + sem controle
    const operacional = presentes + faltas + saiu + entrada + semCtrl;
    ...
    const rows = hist.map((r) => {
      ...
      if (wantFilter && Array.isArray(r._employees) && r._employees.length > 0 && !hasDetails) {
        const emps = r._employees.filter((emp) => {
          ...
          if (deptoFilter && !matchesDeptoFilter(emp, deptoFilter, { nomeToId: fpdNomeToId }))
            return false;
          return true;
        });
        ...
        return {
          ...
          _events: Array.isArray(r._events)
            ? r._events.filter((ev) => {
                ...
                if (deptoFilter && !matchesDeptoFilter(ev, deptoFilter, { nomeToId: fpdNomeToId }))
                  return false;
                return true;
              })
            : null,
  ...
  }, [hist, filialFilter, deptoFilter, matchesFilters, matchesFilial, fpdNomeToId]);
  ...
  const deptChartOptions = useMemo(() => {
    return deptStats
      .map((r) => ({
        value: String(r.id ?? r.idDepartamento ?? r.depto),
        label: r.depto || String(r.id ?? r.idDepartamento ?? "—"),
      }))
  ...
  useEffect(() => {
    ...
    const valid = new Set(deptChartOptions.map((o) => o.value));
  ...
  const deptosRank = useMemo(() => {
    ...
    const rows = deptStats.filter((r) => {
      const key = String(r.id ?? r.idDepartamento ?? r.depto);
      return !useFilter || filterSet.has(key);
    });
